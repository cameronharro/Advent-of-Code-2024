...
export const fullData: inputData = {
    rules: [
        '48|39',
        '39|84',
        '39|23',
...
// Each rule defines a Set of numbers which may not precede the number that is the key in the Map.
type rules = Map<number,Set<number>>;
type instructions = Array<number>

export class Updates {
    rules: rules = new Map();
    edits: Array<{instructions: instructions, valid?: boolean, middle?: number}> = [];
    count: number = 0;
    constructor(input:inputData) {
        input.rules.map(ele => ele.split('|')).forEach(ele => {
            const key = Number(ele[0]);
            const follower = Number(ele[1])
            this.rules.has(key) ? this.rules.get(key)?.add(follower) : this.rules.set(key, new Set([follower]))
        })
        this.edits = input.edits.map(ele => {
            return {
                instructions: ele.split(',').map(ele => Number(ele))
            }
        })
            
        for(let i = 0; i < this.edits.length; i ++) {
            const edits = this.edits[i];
            const line = edits.instructions;
            edits.valid = checkPriorRules(1, line, this.rules)
            if (!edits.valid) {
                reorderInstructions(line, this.rules)
                edits.middle = line[Math.floor(line.length / 2)]
                this.count += edits.middle;
            }
        }
    }
}

export function checkPriorRules (presentIndex: number, line: Array<number>, rules: rules): boolean {
    const presentNumber = line[presentIndex]
    // priorSet contains all numbers which precede presentNumber
    const priorSet = new Set(line.slice(0,presentIndex));
    // ruleSet contains all numbers which may not precede presentNumber
    const ruleSet = rules.get(presentNumber);
    // console.log({line, presentIndex, presentNumber, priorSet, ruleSet})

    // if there is any overlap between priorSet and ruleSet, return false because this whole line is invalid.
    // else move on to check the next number in the line until reaching the end, then return true
    if (!ruleSet || priorSet.isDisjointFrom(ruleSet)) { 
        return presentIndex === line.length - 1 || checkPriorRules(presentIndex + 1, line, rules) 
    }
    return false
}

export function reorderInstructions (line: Array<number>, rules: rules): Array<number> {
    return line.sort((a, b) => {
        // ruleSet contains all numbers which may not precede a
        const ruleSet = rules.get(a);
        // if a has no ruleSet, indicate that a and b are equal
        if(!ruleSet) {return 0}
        // if ruleSet contains b, a and b are correctly ordered
        const isValid = ruleSet?.has(b);
        // console.log({a,b,ruleSet, isValid});
        return isValid ? -1 : 1
    })
}
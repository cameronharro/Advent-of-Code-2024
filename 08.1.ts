...
export class SignalMap {
    ...
    #checkIsInsideMap(coordinate: coordinate): boolean {
        const {x, y} = coordinate
        if(x < 0 || y < 0) {return false}
        if(y >= this.grid.length) {return false}
        if(x >= this.grid[y].length) {return false}
        return true
    }
    ...
    #evaluateAntinodes(antennae: antennae) {
        antennae.forEach((ele, i) => {
            ...
            for (const antenna of remainingAntennae) {
                ...
                const antinodes = getAntinodes(ele, antenna, slope).filter(ele => this.#checkIsInsideMap(ele));
...
export function getAntinodes(point1: coordinate, point2: coordinate, slope: slope): Array<coordinate> {
    const antinode1: coordinate = {x: point1.x - slope.run, y: point1.y - slope.rise};
    const antinode2: coordinate = {x: point2.x + slope.run, y: point2.y + slope.rise};

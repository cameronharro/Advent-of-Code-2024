...
export class Plant {
    ...
    constructor(input: Array<string>) {
        this.reports = formatData(input);
        this.#keyMath();
        this.reports.forEach((ele, i) => {
            ele.index = i;
            test(ele)
        })
        this.reports.forEach(ele => ele.isSafe ? this.countSafe ++ : null)
    }
    ...
    #keyMath () {
        for (const report of this.reports) {
            ...
            report.first = data[0];
            report.last = data[data.length - 1];
            report.first > report.last
            ? report.direction = 'descending'
            : report.last > report.first
                ? report.direction = "ascending"
                : report.direction = "static"
        }
    }
...
function test (report: Report) {
    // const range = Math.abs(Number(report.first) - Number(report.last));
    // const length = report.data.length;
    // if(range < length - 1 || range > ((length - 1) * 3)) {
    //     report.isSafe = false
    // } else {
        report.isSafe = analyzeData(report.data, report.direction);
    // }
    return report
}
...
function analyzeData (
    ...
    for(let i = 1; i < length; i ++) {
        ...
        if(direction === 'ascending' && (diff < 1 || diff > 3)) { 
            if (failed){ return false }
            const removePrevIsSafe = analyzeData(data.toSpliced(i - 1, 1), direction, true);
            // console.log({removePrevIsSafe})
            if(removePrevIsSafe) {
                return true
            } else {
                const removeCurrIsSafe = analyzeData(data.toSpliced(i, 1), direction, true);
                // console.log({removeCurrIsSafe})
                if(removeCurrIsSafe) { return true}
            }
            return false
        } else if(direction === 'descending' && (diff > -1 || diff < -3)) {
            if (failed){ return false }
            const removePrevIsSafe = analyzeData(data.toSpliced(i - 1, 1), direction, true);
            // console.log({removePrevIsSafe})
            if(removePrevIsSafe) {
            ...
            return false
        } else if(direction === 'static') { return false }
    }
    return true;
}

function formatData(input: Array<string>) {
    return input.map(ele => ele.split(' ').map(ele => Number(ele))).map(ele => {
        return {data: ele}
    });
}